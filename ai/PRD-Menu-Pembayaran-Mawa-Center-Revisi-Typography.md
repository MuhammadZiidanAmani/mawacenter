...
Semua desain wajib mengikuti [standar aplikasi.md]\(standar aplikasi.md), terutama standar halaman transaksi, daftar tagihan, form 40px, tombol primer hijau, warna netral, sistem typography 12/14/16/20px, dan responsif mobile.
...
Ikuti standar aplikasi dengan hierarchy visual yang tenang, profesional, dan konsisten untuk aplikasi administrasi keuangan. Hindari terlalu banyak elemen bold, terlalu banyak warna aksen, atau ukuran teks yang saling bersaing.
...
\- Panel lembut/empty state/selected soft: #fbfdf8.

\- Teks utama / primary text: #0f172a.

\- Teks sekunder / secondary text: #334155.

\- Teks muted / metadata: #64748b.

\- Teks disabled: #94a3b8.

\- Hijau utama: #004528.

\- Hover hijau: #0d5f36.

\- Hijau aksen teks/status jika diperlukan: #047857.

Aturan warna:

\- Gunakan hijau terutama untuk CTA utama, selected state, status positif, focus state, dan total transaksi utama.

\- Jangan membuat seluruh nominal, seluruh heading, atau seluruh metadata berwarna hijau.

\- Nominal item tagihan biasa menggunakan teks utama; hanya total pembayaran utama yang boleh diberi penekanan hijau.

**### Field dan Tombol**

\- Field tinggi 40px, radius 8px.

\- Tombol standar tinggi 40px, radius 8px.

\- Tombol CTA utama boleh 44px jika membutuhkan penekanan lebih kuat.

\- Border field default #d1d5db.

\- Focus state menggunakan hijau utama dengan ring/border yang jelas tetapi tidak terlalu tebal.

**### Sistem Typography**

Gunakan maksimal empat ukuran font utama: 12px, 14px, 16px, dan 20px.

\- **12px / 600**: hanya untuk eyebrow, micro-label, label status kecil, atau teks kategori singkat seperti `SISWA DIPILIH`. Jangan digunakan untuk body text, input, atau informasi transaksi utama.

\- **14px / 400**: body text, metadata, periode, helper text, dan informasi sekunder.

\- **14px / 500**: label form seperti `Bayar sampai`, `Nominal Dibayar`, dan `Metode Pembayaran`.

\- **14px / 600**: section heading kecil, badge text, serta label operasional yang perlu sedikit penekanan.

\- **16px / 600**: judul card, nama siswa, nama tagihan, nominal item tagihan, dan informasi utama level menengah.

\- **20px / 700**: judul halaman dan total transaksi utama.

Hindari penggunaan 700 secara berlebihan. Gunakan 700 hanya untuk judul halaman, total transaksi utama, atau informasi yang memang menjadi hierarchy tertinggi.

**### Font Weight**

\- 400: body, metadata, periode, helper text.

\- 500: label form.

\- 600: card heading, nama siswa, nama tagihan, section heading, tombol, dan nominal item.

\- 700: page title dan total pembayaran utama.

Jangan menggunakan 700 untuk seluruh label, seluruh card title, dan seluruh nominal secara bersamaan karena membuat halaman terasa terlalu berat.
...
Nama siswa atau teks panjang boleh wrap ke baris berikutnya dengan line-height yang sama dan tidak boleh dipaksa satu baris jika menyebabkan overflow.

**### Angka dan Nominal**

\- Gunakan `font-variant-numeric: tabular-nums` untuk angka rupiah, saldo, nominal transaksi, dan data numerik yang sering disejajarkan.

\- Nominal item tagihan: 16px / 600, warna teks utama.

\- Total pembayaran utama: 20px / 700, boleh menggunakan hijau utama.

\- Metadata nominal atau informasi tambahan: 14px / 400 atau 500.

\- Prefix `Rp` harus konsisten dan tidak mengubah nilai numerik yang dikirim ke backend.

**### Hierarchy Komponen Pembayaran**

Contoh hierarchy yang menjadi acuan:

\- `Pembayaran`: 20px / 700 / primary text.

\- Deskripsi halaman: 14px / 400 / secondary text.

\- `SISWA DIPILIH`: 12px / 600 / accent green.

\- Nama siswa: 16px / 600 / primary text.

\- NIS, unit, kelas: 14px / 400 / muted text.

\- `Daftar Tagihan`: 16px / 600 / primary text.

\- `Tagihan Wajib`: 14px / 600 / primary text.

\- Nama tagihan: 16px / 600 / primary text.

\- Periode tagihan: 14px / 400 / muted text.

\- `Bayar sampai`: 14px / 500 / secondary text.

\- Nominal item: 16px / 600 / primary text.

\- `Ringkasan Pembayaran`: 16px / 600 / primary text.

\- Label form ringkasan: 14px / 500 / secondary text.

\- Total pembayaran utama: 20px / 700 / primary green.

\- Tombol: 14px / 600.

**### Prinsip Konsistensi**

\- Jangan menggunakan ukuran font baru di luar 12/14/16/20px tanpa kebutuhan khusus yang terdokumentasi.

\- Jangan menggunakan font weight selain 400/500/600/700 tanpa alasan khusus.

\- Heading card tidak boleh lebih dominan daripada judul halaman.

\- Nama siswa dan nama tagihan harus lebih dominan daripada metadata, tetapi tidak lebih dominan daripada total pembayaran utama.

\- Perbedaan hierarchy sebaiknya dibangun dari kombinasi ukuran, weight, warna, dan spacing; bukan dengan membesarkan semua teks penting.
