        ...
        <main class="finance-page">
            ...
            <div>
                ...
                <section class="card other-import-mapping">
                    <div class="spp-preview-table-head"><div><strong>Pemetaan Jenis Pembayaran</strong><span>Pastikan kategori Excel diarahkan ke jenis pembayaran aplikasi yang benar untuk setiap unit.</span></div><span class="spp-preview-count">{{ count($importSources) }} pemetaan</span></div>
                ...
                <section class="card master-card spp-history other-payment-history">
                    ...
                    <div class="table-wrap"><table @class(['data-table', 'spp-list-table', 'registration-payment-table' => $paymentSection['key'] === 'daftar-ulang'])><thead><tr><th>No</th><th>@include('partials.sortable-heading', ['column' => 'nis', 'label' => 'NIS'])</th><th>@include('partials.sortable-heading', ['column' => 'name', 'label' => 'Nama'])</th><th>@include('partials.sortable-heading', ['column' => 'unit', 'label' => 'Unit Pendidikan'])</th><th>@include('partials.sortable-heading', ['column' => 'class', 'label' => 'Kelas'])</th><th>@include('partials.sortable-heading', ['column' => 'method', 'label' => 'Cara Bayar'])</th><th>@include('partials.sortable-heading', ['column' => 'total', 'label' => 'Total'])</th><th></th></tr></thead><tbody>
                        @forelse($payments as $payment)
                            <tr class="spp-main-row">
                                <td>{{ $payments->firstItem()+$loop->index }}</td><td>{{ $payment->student?->nis ?? '-' }}</td><td><strong>{{ $payment->student?->name ?? '-' }}</strong></td><td>@if($paymentSection['key'] === 'daftar-ulang')<span class="education-code">{{ $payment->student?->schoolClass?->educationUnit?->code ?? '-' }}</span>@else{{ $payment->student?->schoolClass?->educationUnit?->name ?? '-' }}@endif</td><td>{{ $payment->student?->schoolClass?->name ?? '-' }}</td><td><span class="payment-method">{{ strtolower($payment->payment_method) }}</span></td><td><strong>Rp {{ number_format($payment->paid_amount,0,',','.') }}</strong></td><td><button type="button" class="spp-expand-button" data-spp-row-toggle="other-{{ $payment->id }}" aria-expanded="false">+</button></td>
                            </tr>
                            <tr class="spp-expanded-row" data-spp-row-detail="other-{{ $payment->id }}" hidden><td colspan="8">
                                @if($paymentSection['key'] === 'daftar-ulang')
                                    <div class="registration-payment-detail">
                                        <div class="registration-detail-item payment-type"><span>Jenis Pembayaran</span><strong>{{ $payment->feeType?->name ?? '-' }}</strong></div>
            ...
            <section class="spp-form-page payment-create-page">
                <section class="hero master-hero"><div><p class="eyebrow">Pembayaran · {{ $paymentSection['title'] }}</p><h1>Tambah Pembayaran {{ $paymentSection['title'] }}</h1><p>Pilih siswa dan jenis pembayaran yang berlaku untuk kelasnya.</p></div><a href="{{ route('finance.other.index', ['category' => $paymentSection['key']]) }}" class="button button-secondary">Kembali ke Daftar</a></section>
            ...
            <div class="modal-backdrop" data-other-edit-modal>
                <div class="form-modal spp-edit-modal">
                    <div class="form-modal-header"><div><p class="eyebrow">Pembayaran · Daftar Ulang</p><h2>Edit Transaksi</h2></div><button type="button" class="icon-button" data-other-crud-close>×</button></div>
                    <form method="POST" data-other-edit-form class="master-form spp-edit-form">@csrf @method('PUT')
                        <div class="spp-edit-readonly"><span data-other-edit-summary>Data siswa dan jenis pembayaran tidak dapat diubah.</span></div>
