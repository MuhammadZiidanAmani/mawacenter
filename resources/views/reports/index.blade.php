        ...
        <main class="finance-page report-page">
            ...
            <section class="card report-filter-card">
                ...
                <form method="GET" class="report-filter">
                    ...
                    <label>Kategori pembayaran<select name="type"><option value="">Semua Kategori</option><option value="spp" @selected($filters['type']==='spp')>SPP</option><option value="other" @selected($filters['type']==='other')>Lain-lain</option></select></label>
            ...
            <section class="report-grid">
                ...
                <div class="card report-composition"><div class="report-section-head"><span>{!! $icon('receipt') !!}</span><div><strong>Komposisi Pemasukan</strong><small>Perbandingan kategori pembayaran</small></div></div><div class="composition-total"><strong>{{ $rupiah($stats['total']) }}</strong><span>total pemasukan</span></div><div class="composition-row"><span><i class="spp"></i>SPP</span><strong>{{ $stats['total'] ? round($stats['spp']/$stats['total']*100) : 0 }}%</strong></div><div class="composition-row"><span><i class="other"></i>Lain-lain</span><strong>{{ $stats['total'] ? round($stats['other']/$stats['total']*100) : 0 }}%</strong></div></div>
            </section>
            <section class="card report-table-card">@include('partials.list-toolbar', ['action' => route('reports.index'), 'searchLabel' => 'Cari transaksi siswa'])<div class="table-wrap"><table class="data-table report-table"><thead><tr><th>@include('partials.sortable-heading', ['column' => 'date', 'label' => 'Tanggal'])</th><th>@include('partials.sortable-heading', ['column' => 'student', 'label' => 'Siswa'])</th><th>@include('partials.sortable-heading', ['column' => 'type', 'label' => 'Jenis'])</th><th>@include('partials.sortable-heading', ['column' => 'description', 'label' => 'Keterangan'])</th><th>@include('partials.sortable-heading', ['column' => 'unit', 'label' => 'Unit / Kelas'])</th><th>@include('partials.sortable-heading', ['column' => 'method', 'label' => 'Metode'])</th><th>@include('partials.sortable-heading', ['column' => 'amount', 'label' => 'Nominal'])</th></tr></thead><tbody>@forelse($transactions as $item)<tr><td><strong>{{ $item['date']->format('d/m/Y') }}</strong><small>{{ $item['date']->format('H:i') }}</small></td><td><strong>{{ $item['student'] }}</strong><small>{{ $item['nis'] }}</small></td><td><span class="report-type {{ $item['type']==='SPP'?'spp':'other' }}">{{ $item['type'] }}</span></td><td>{{ $item['description'] }}</td><td><strong>{{ $item['unit'] }}</strong><small>{{ $item['class'] }}</small></td><td>{{ $item['method'] }}</td><td><strong class="report-amount">{{ $rupiah($item['amount']) }}</strong></td></tr>@empty<tr><td colspan="7" class="empty-state">Belum ada transaksi pada filter ini.</td></tr>@endforelse</tbody></table></div><div class="pagination-wrap">{{ $transactions->links() }}</div></section>
        </main>
    </div>
</div>
</body>
