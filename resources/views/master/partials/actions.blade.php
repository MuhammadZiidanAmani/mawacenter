...
<div @class(['table-actions', 'student-ghost-actions' => $isStudentAction, 'student-card-actions' => $isStudentCardAction])>
    ...
    <form method="POST" action="{{ route('master.destroy', $deleteParameters) }}" onsubmit="return confirm('Hapus data ini? Data yang dihapus tidak dapat dikembalikan.')">@csrf @method('DELETE')<button class="icon-button delete-button" title="Hapus" aria-label="Hapus data"><svg class="icon" viewBox="0 0 24 24"><path d="M3 6h18M8 6V4h8v2m-9 0 1 15h8l1-15M10 11v5m4-5v5"/></svg></button></form>
    @endif
</div>
